...
(* 程序是否已经运行，如果运行则激活它 *)
function AppHasRun(AppHandle: THandle; AppName: string): Boolean;
...
procedure CreateMapFile(myMapFile: string = MapFileName);
begin
  hMapFile := OpenFileMapping(FILE_MAP_ALL_ACCESS, False, PChar(myMapFile));
  if hMapFile = 0 then
  begin
  //32位
//    hMapFile := CreateFileMapping($FFFFFFFF, nil, PAGE_READWRITE, 0,
//      SizeOf(TShareMem), MapFileName);
    // Modified by cyq 2020-12-27 17:37:43  兼容64位
    hMapFile := CreateFileMapping($FFFFFFFFFFFFFFFF, nil, PAGE_READWRITE, 0,
      SizeOf(TShareMem), PChar(myMapFile));
  ...
  begin
    ...
    begin
      ...
      if (TopWindow <> 0) and (TopWindow <> PSMem^.AppHandle) and
        IsWindowVisible(TopWindow) and IsWindowEnabled(TopWindow)
      then
